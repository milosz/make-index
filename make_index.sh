...
# ""            - use root directory
# "/downloads"  - use '/downloads' subdirectory
root_directory="/home/milosz/downloads/make_index"
...
find . -type d -not -path "*/img" -not -path "*/css" | while read directory; do
  ...
  do
    if [ -d "$directory/$file" ]; then
      ...
      content+="<div class=\"row\">"
      if [ "$directory" == "." ]; then
        content+="<div class=\"span10\"><div class=\"file\"><a href=\"$root_directory/$file\">$file</a></div></div>"
      else
        content+="<div class=\"span10\"><div class=\"file\"><a href=\"$root_directory/$directory/$file\">$file</a></div></div>"
      fi    
